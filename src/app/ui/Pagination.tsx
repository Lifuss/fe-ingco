...
function PaginationNumber({
  ...
  const className = clsx(
    'flex h-7 w-7 md:h-10 md:w-10 items-center justify-center text-sm border',
    {
      ...
      ' bg-orange-light border-orange-light text-white': isActive && !isDashboard,
  ...
  return isActive || position === 'middle' ? (
    ...
    <Link href={href} className={className} aria-label={`Перейти на сторінку ${page}`}>
      {page}
    </Link>
  );
}
...
function PaginationArrow({
  ...
  return isDisabled ? (
    ...
    <Link
      ...
      href={href}
      aria-label={
        direction === 'left' ? 'Перейти на одну сторінку назад' : 'Перейти на одну сторінку вперед'
      }
    >
      {icon}
    </Link>
