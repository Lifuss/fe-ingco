...
const Header = () => {
  ...
  return (
    <header className="bg-orange-light flex min-w-0 items-center justify-between gap-2 overflow-hidden px-3 py-2 font-medium md:px-[60px] md:py-4 lg:tracking-tight">
      <Link
        href={isShop ? '/shop' : '/'}
        className={clsx(
      ...
      {pathname.includes('/home') ? (
        <ul className="min-w-0 flex-1 text-lg md:flex md:items-center md:gap-2 md:text-base lg:text-[20px] xl:gap-10 2xl:text-2xl">
          ...
          <li className="flex">
            ...
            <Link className="pl-1 transition-colors ease-out hover:text-white" href="/shop">
              Гуртом
            </Link>
          </li>
        </ul>
            ...
            <>
              ...
              <div className="flex items-center justify-center gap-2">
                ...
                <Link href={isShop ? '/shop/cart' : '/cart'} className="relative">
                  ...
                  />
                  {(isShop && user.cart.length) || (!isShop && user.retailCart?.length) ? (
